...
def get_mol(smiles: str):
    """
    Get the rdkit molecule objct from a SMILES string, inverse method to get_smiles()
    :param smiles: string represeting molecule in SMILES format
...
def get_smiles(mol):
    """
    Get the SMILES string from rdkit molecule objct, inverse method to get_mol()
    :param mol: rdkit molecule object

    :return: string SMILES molecule
    """
    return Chem.MolToSmiles(mol, kekuleSmiles=True)


def sanitize(mol, kekulize: bool = True):
    try:
...
def atom_equal(a1, a2):
    """
    Given two atoms check if they are equal: check their symbol and Formal charge
    :param a1: atom
    :param a2: atom
    :return: boolean value checking if atoms are equal
    """
    return (
